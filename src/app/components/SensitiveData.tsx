...
export function SensitiveData() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems(all.filter((i: any) => i.Riscos && i.Riscos !== "Nenhum" && i.Riscos !== "" && i.risk_level !== "none"));
    } catch (e: any) {
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Rissco detectado</TableHead>
            ...
            <TableBody>
              {items.map((item, i) => {
                const risco = item.Riscos || item.risk_level || "desconhecido";
                const cls = RISK_STYLE[risco] || "bg-gray-100 text-gray-700 border-gray-200";
                return (
                  <TableRow key={i}>
                    <TableCell className="font-medium max-w-[200px] truncate">{item.Arquivo || item.name}</TableCell>
                    <TableCell><span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${cls}`}>{ItemElement}</span></TableCell>
                    <TableCell className="text-muted-foreground text-xs max-w-[280px] truncate">{item.Caminho || item.path}</TableCell>
