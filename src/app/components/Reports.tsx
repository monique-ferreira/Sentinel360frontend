      ...
      {loading ? (
        ...
        <div className="py-16 text-center text-muted-foreground">
          {search ? (
            ...
            <><CheckCircle2 className="h-10 w-10 mx-auto mb-3 text-green-500 opacity-60"/><p className="font-medium">Nenhum arquivo detectado ainda</p><p className="text-sm mt-1">Va em Integracoes e clique em Iniciar Scan Real</p></>
          )}
        </div>
      ) : (
        <Card>
          <CardContent className="p-0">
            <Table>
              <TableHeader><TableRow>
                ...
                <TableHead>Riscos</TableHead><TableHead>Data scan</TableHead>
              ...
              <TableBody>
                {filtered.map((item, i) => {
                  ...
                  const cls = Object.entries(RISK_STYLE).find(([k]) => risco.includes(k))?.[1] || "bg-gray-100 text-gray-600 border-gray-200";
                  const path = item.caminho || item.Caminho || item.path;
                  const nome = item.nome || item.Arquivo || item.name;
                  const data = item.last_scan || item.Data;
                  ...
                  return (
                    <TableRow key={i}>
                      ...
                      <TableCell>{risco && risco !== "NENHUM" && risco !== "Nenhum" ? <span className={`inline-flex px-2 py-0.5 rounded-full text-xs border ${cls}`}>{risco}</span> : <span className="text-muted-foreground text-xs">—</span>}</TableCell>
