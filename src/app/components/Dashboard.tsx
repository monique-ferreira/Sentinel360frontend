...
export function Dashboard() {
  ...
  useEffect(() => { load(); }, []);
  // Fields: nome, caminho, inativo ("SIM"/"NÃO"), riscos, tamanho_mb, last_scan
  ...
  const topRiscos = items.filter(i => {
    const r = i.riscos || i.Riscos || "";
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {!error && !loading && total === 0 && <div className="flex items-center gap-2 p-3 rounded-lg bg-blue-50 border border-blue-200 text-sm text-blue-700"><Wifi className="h-4 w-4 shrink-0"/>Nenhum scan realizado ainda. Va em <strong>Integracoes</strong> e clique em Iniciar Scan Real.</div>}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard title="Com riscos" value={loading ? "—" : comRiscos.toLocaleString("pt-BR")} subtitle="credenciais / tokens" icon={<ShieldAlert className="h-5 w-5"/>} color="danger" />
              ...
              {topRiscos.map((item, i) => (
                <div key={i} className="flex items-center justify-between px-4 py-3">
                  ...
                  <div className="text-right shrink-0 ml-3">
                    <span className="inline-block text-xs px-2 py-0.5 rounded-full bg-red-100 text-red-700 border border-red-200">{item.riscos || item.Riscos || item.risk_level}</span>
                    <p className="text-xs text-muted-foreground mt-0.5">{item.tamanho_mb ?? item.Tamanho_MB ?? item.size_mb} MB</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {loading && <div className="flex items-center justify-center py-12 text-muted-foreground gap-2"><RefreshCw className="h-5 w-5 animate-spin"/>Conectando ao servidor...</div>}
    </div>
  );
}