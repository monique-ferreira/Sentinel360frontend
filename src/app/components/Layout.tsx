...
export function Layout() {
  ...
  const isPending = userProfile?.account_type === "corporate" && userProfile?.org_status === "pending";

  const NAV = (() => {
    if (isPending) return BASE_NAV.filter(n => n.to === "/");
    let nav = isAdmin
      ? [...BASE_NAV, { to: "/workspace", icon: Building2, label: "Workspace", end: false, tour: undefined }]
      : BASE_NAV;
    if (isMember) nav = nav.filter(n => n.to !== "/reports" && n.to !== "/sensitive-data");
    return nav;
  })();
  ...
  useEffect(() => {
    if (isPending && location.pathname !== "/") {
      navigate("/");
    } else if (isMember && (location.pathname === "/reports" || location.pathname === "/sensitive-data")) {
      navigate("/");
    }
  }, [isPending, isMember, location.pathname]);
