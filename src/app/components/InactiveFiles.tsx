...
export function InactiveFiles() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems(all.filter((i: any) => i.Inativo === "SIM" || i.is_inactive === true));
    } catch (e: any) {
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold flex items-center gap-2"><FolderClock className="h-6 w-6"/> Arquivos Inativos</h1>
          <p className="text-sm text-muted-foreground">Arquivos sem acesso ha mais de 180 dias</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="py-16 text-center text-muted-foreground">
          <FolderClock className="h-10 w-10 mx-auto mb-3 opacity-40"/>
          <p className="font-medium">Nenhum arquivo inativo encontrado</p>
          <p className="text-sm mt-1">Execute um scan para detectar arquivos inativos</p>
        </div>
      ) : (
        <div className="border rounded-lg overflow-hidden">
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Data do scan</TableHead>
            ...
            <TableBody>
              {items.map((item, i) => (
                <TableRow key={i}>
                  ...
                  <TableCell className="text-muted-foreground text-xs max-wl[300px] truncate">{item.Caminho || item.path}</TableCell>
          ...
          <div className="px-4 py-2 border-t text-xs text-muted-foreground">{items.length} arquivo(s) inativo(s)</div>
        </div>
      )}
    </div>
  );
