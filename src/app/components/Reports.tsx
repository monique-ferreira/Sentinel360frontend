...
export function Reports() {
  ...
  const handleDelete = async () => {
    ...
    const path = confirmItem.caminho || confirmItem.Caminho || confirmItem.path;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setDeleteError(data?.detail ?? `Erro ${res.status} ao excluir.`);
        return;
      }
      setDeleted(p => [...p, path]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-2xl border border-border bg-card shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="rounded-lg bg-secondary border border-border px-3 py-2.5">
                <p className="text-xs text-muted-foreground mb-0.5">Arquivo</p>
                <p className="text-sm font-medium text-foreground truncate">
                  {confirmItem.nome || confirmItem.Arquivo || confirmItem.name}
                </p>
                <p className="text-xs text-muted-foreground truncate mt-0.5">
                  {confirmItem.caminho || confirmItem.Caminho || confirmItem.path}
                </p>
              </div>
              ...
              <div className="flex gap-2 pt-1">
                <button
                  onClick={() => { setConfirmItem(null); setDeleteError(""); }}
                  disabled={!!deleting}
                  className="flex-1 h-9 rounded-lg border border-border bg-secondary text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-white/5 disabled:opacity-50 transition-colors">
                  Cancelar
                </button>
                <button
                  onClick={handleDelete}
                  disabled={!!deleting}
                  className="flex-1 h-9 rounded-lg bg-destructive text-sm font-medium text-white hover:bg-destructive/90 disabled:opacity-50 transition-colors flex items-center justify-center gap-2">
                  {deleting ? <><RefreshCw className="w-3.5 h-3.5 animate-spin" />Excluindo...</> : <><Trash2 className="w-3.5 h-3.5" />Excluir</>}
                </button>
              </div>
            </div>
          </div>
                ...
                return (
                  <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => { setConfirmItem(item); setDeleteError(""); }} disabled={deleting === path}
